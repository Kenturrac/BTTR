...
private ["_missName","_coords","_direction","_startCoords","_plane","_aiGrp","_pilot","_wp","_wp_pos","_loop","_half","_newPos","_plane2","_chute","_box","_dropDir","_wp2","_fallCount","_boxFin"];
...
[nil,nil,rTitleText,"An Ikeabox Testplain with Bandit Supplies is Flying In!\nGet to the Location and Capture the Package!", "PLAIN",10] call RE;
...
switch (_direction) do{
	//north
	//_startCoords = [random 16500, 15350, 500] ;
	//east
	case 0: {_startCoords = [16500, random 15350, 500]; };
	//south
	case 1: {_startCoords = [random 16500, 0, 500]; };
	//west
	//_startCoords = [0, random 15350, 500];
};

_plane = createVehicle ["GNT_C185F", _startCoords, [], 0, "FLY"];
...
while {_loop} do {
	if (!Alive _plane OR !Alive _pilot) then {
		...
		_plane2 = createVehicle ["MV22_DZ", [(_newPos select 0),(_newPos select 1),200], [], 0, "FLY"];
	...
	if ((Alive _plane) AND (Alive _pilot) AND ((_plane distance _wp_pos) <= 1200) AND (!(_half))) then {
		[nil,nil,rTitleText,"The Ikeabox Testplain is only 1200m out from the drop point!", "PLAIN",10] call RE;
		
		//Keep on truckin'
		_plane forceSpeed 175;
		_plane flyInHeight 110;
	...
	if ((Alive _plane) AND (Alive _pilot) AND ((_plane distance _wp_pos) <= 200)) then {
		...
		diag_log ("DEBUG-IKEA: _newPos: " + str(_newPos));
...
[_boxFin,"Ikea"] ExecVM DZMSBoxSetup;
...
diag_log text format["[DZMS]: Major SM1 Ikeabox Testplain Drop Mission has Ended."];
...
//Kill plane if it's back at the map border

if ((_plane distance _wp2) <= 200) then {
	deleteVehicle _plane;
	deleteVehicle _pilot;
}